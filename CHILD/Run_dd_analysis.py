...
# by DataDirectory and then runs the drainage
# density analysis for all CHILD runs.  The 
# file generate_dd_drivers.py must be run first 
# to get driver files for each ENVI file.
#=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
# FJC 07/07/15
#=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
...
for FileName in glob(DataDirectory+"*drainage_density_child*.driver"): 
    ...
    system_call = "nice ./child_drainage_density.out " + DataDirectory + " " + split_fname
